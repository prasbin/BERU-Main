...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".beru_profiles" / browser
...
def _resolve_profile_dir_name(user_data_dir: Path, profile_display_name: str) -> str:
    ...
    try:
        ...
        for dir_name, info in profiles.items():
            if info.get("name", "").lower() == profile_display_name.lower():
                print(f"[Browser] Mapped display name '{profile_display_name}' to directory '{dir_name}'")
                return dir_name
    except Exception as e:
        print(f"[Browser] Error reading Local State: {e}")
    return profile_display_name
...
class _BrowserSession:
    ...
    def __init__(self, browser_name: str, profile_name: str | None = None):
    ...
    def start(self):
        if self._thread and self._thread.is_alive():
            return
        thread_name = f"BrowserThread-{self.browser_name}"
        if self.profile_name:
            thread_name += f"-{self.profile_name}"
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".beru_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using BERU profile")
                beru = str(Path.home() / ".beru_profiles" / "firefox_beru")
                Path(beru).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(beru, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".beru_profiles" / "safari")
        ...
        if self.profile_name:
            resolved_profile = _resolve_profile_dir_name(Path(profile), self.profile_name)
            args.append(f"--profile-directory={resolved_profile}")
        ...
        if self.profile_name:
            label += f" profile={self.profile_name}"
        ...
        beru_profile = str(Path.home() / ".beru_profiles" / self.browser_name)
        Path(beru_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with BERU profile: {beru_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(beru_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with BERU profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "beru_screenshot.png")
...
class _SessionRegistry:
    ...
    def _get_or_create(self, browser_name: str, profile_name: str | None = None) -> _BrowserSession:
        key = f"{browser_name}:{profile_name}" if profile_name else browser_name
        with self._lock:
            if key not in self._sessions:
                sess = _BrowserSession(browser_name, profile_name)
                sess.start()
                self._sessions[key] = sess
                print(f"[Registry] New session: {key}")
            return self._sessions[key]

    def get(self, browser_name: str | None = None, profile_name: str | None = None) -> _BrowserSession:
        if not browser_name:
        ...
        sess = self._get_or_create(browser_name, profile_name)
        self._active_browser = browser_name
        return sess

    def switch(self, browser_name: str, profile_name: str | None = None) -> str:
        browser_name = _ALIASES.get(browser_name.lower().strip(), browser_name.lower().strip())
        self._get_or_create(browser_name, profile_name)
        self._active_browser = browser_name
        return f"Active browser → {browser_name}"

    def close_one(self, browser_name: str, profile_name: str | None = None) -> str:
        key = f"{browser_name}:{profile_name}" if profile_name else browser_name
        with self._lock:
        ...
        if sess:
            ...
            return f"{key} closed."
        return f"No active session for: {key}"
...
def browser_control(
    ...
    profile = params.get("profile", "").strip() or None
    result  = "Unknown action."

    if action == "switch":
        target = browser or params.get("target", "").lower().strip()
        result = _registry.switch(target, profile) if target else "Please specify a browser."
        _log(player, result)
        return result
    ...
    try:
        sess = _registry.get(browser, profile)
    except Exception as e:
    ...
    try:
        if action == "go_to":
            ...
            result = _registry.close_one(target, profile) if target else "No browser specified."
        else:

...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"beru_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"beru_debug_{int(time.time())}.png"
...
def _explain_action(file_path, code, player, mode="summary") -> str:
    ...
    if mode == "algorithm":
        prompt = f"""Explain the algorithmic approach used in this code.
Cover: the core approach/technique used, why it works, and its time and space complexity
(Big-O). Be precise and technical, but clear. 4 to 8 sentences, or a short structured
breakdown if multiple distinct steps/algorithms are involved.
...
Algorithm explanation:"""
        ...
        prompt = f"""Give a line-by-line (or block-by-block for repetitive/boilerplate sections)
breakdown of what this code does. Reference each line or short block, then explain its purpose
in one short sentence. Skip purely cosmetic lines (blank lines, simple imports) unless they
matter to understanding the logic.
...
def _debug_action(file_path, code, error_output, description, language, player, speak=None) -> str:
    if file_path and not code:
        code, err = _read_file(file_path)
        if err:
            return err
    if not code:
        return "Please provide code or a file path to debug, sir."

    if player:
        player.write_log("[Code] Debugging...")

    runnable_ext = (".py", ".js", ".ts", ".sh", ".ps1", ".rb", ".php")

    # If no error was supplied, try to reproduce one by actually running the file.
    if not error_output and file_path:
        p = Path(file_path)
        if p.exists() and p.suffix.lower() in runnable_ext:
            print("[Code] 🐞 No error given — running file to reproduce one...")
            run_output = _run_file(p, [], 30)
            if _has_error(run_output):
                error_output = run_output

    if not error_output:
        return (
            "No error was given, and I couldn't reproduce one by running the file, sir. "
            "Please share the exact error message or traceback you're seeing."
        )

    try:
        fixed = _fix_code(code, error_output, description or "Fix the reported error.")
    except Exception as e:
        return f"Could not debug code: {e}"

    save_path = Path(file_path) if file_path else _resolve_save_path("", language or "python")
    status    = _save_file(save_path, fixed)
    print(f"[Code] ✅ Debugged: {save_path}")

    msg = f"Bug fixed. {status}\n\nError addressed:\n{error_output[:300]}\n\nPreview:\n{_preview(fixed)}"
    if speak:
        speak("Bug fixed, sir. The corrected code has been saved.")
    return msg
...
def code_helper(
    ...
    parameters:
        action      : write | edit | explain | run | build | screen_debug | optimize | debug | auto
        ...
        file_path   : Path to existing file (edit / explain / run / build / optimize / debug)
        code        : Raw code string (explain/optimize/debug without a file)
        args        : CLI argument list for run/build
        timeout     : Execution timeout in seconds (default: 30)
        mode        : For action="explain" — summary (default) | algorithm | line_by_line
        error_output: For action="debug" — the error/traceback to fix (auto-run to reproduce if omitted and file_path is given)
    """
    p            = parameters or {}
    ...
    if action == "write":
        ...
        return _explain_action(file_path, code, player, mode=mode)
        ...
        return _debug_action(file_path, code, error_output, description, language, player, speak)

    elif action == "screen_debug":
        return _screen_debug_action(description, file_path, player, speak)

    else:
        return f"Unknown action: '{action}'. Use write, edit, explain, run, build, optimize, debug, or screen_debug."
...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        url = simpledialog.askstring("BERU", prompt_text, parent=root)
...
def _summarize_with_gemini(transcript: str, video_url: str) -> str:
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=(
                "You are BERU, the Shadow Monarch's assistant — calm, loyal, intelligent, and formal. "
...
def _handle_playlist(parameters: dict, player, speak) -> str:
    playlist_name = parameters.get("playlist_name", "LinkedIn Profile Enhancement Playlist").strip()
    ...
    if not urls:
        ...
        if query:
            ...
            if url:
                urls = [url]
        if not urls:
            return "No video URLs or query provided for the playlist, sir."

    save_local = parameters.get("save_local", True)
    desktop = Path.home() / "Desktop"
    desktop.mkdir(parents=True, exist_ok=True)
    clean_name = re.sub(r'[\\/*?:"<>|]', "", playlist_name).replace(" ", "_").lower()
    filepath = desktop / f"playlist_{clean_name}.md"

    content = f"# YouTube Playlist: {playlist_name}\n"
    content += f"Created by BERU on {datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
    for i, url in enumerate(urls, 1):
        ...
        content += f"{i}. **[{title}]({url})** (by {channel})\n"

    filepath.write_text(content, encoding="utf-8")

    msg = f"Playlist saved to Desktop: {filepath.name}"
    if player:
        player.write_log(f"[YouTube] Playlist created: {playlist_name}")
    if speak:
        speak(f"Master, I have saved the videos to a playlist named '{playlist_name}' on your Desktop.")
    return msg

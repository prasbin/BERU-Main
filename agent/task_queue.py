...
class Task:
    ...
    mission_id:  str        = field(compare=False, default="")
...
class TaskQueue:
    ...
    def _run_task(self, task: Task) -> None:
        ...
        try:
            ...
            if task.mission_id:
                from core.delegation import finalize_mission
                if task.cancel_flag.is_set():
                    finalize_mission(task.mission_id, success=False, result="Cancelled")
                else:
                    finalize_mission(
                        task.mission_id,
                        success=True,
                        result=str(result)[:4000] if result else "Completed",
                    )
            ...
            if task.mission_id:
                from core.delegation import finalize_mission
                finalize_mission(task.mission_id, success=False, result=str(e))
